...
*/ 
    ...
    function completeTask(uint256 task_ID, string memory task_tags) onlyOwner public returns (bytes32 task_hash) {
        task_hash = sha256(abi.encodePacked(task_ID, task_tags));
        uint256 token_ID = getPrimaryTokenID(msg.sender);
        uint256 tasks_completed = characters[token_ID].tasks_completed;
        uint256 task_timestamp = block.timestamp;

        characters[token_ID].task_IDs[tasks_completed] = task_ID;
        characters[token_ID].task_tags_array[tasks_completed] = task_tags;
        characters[token_ID].task_hashes[tasks_completed] = task_hash;
        characters[token_ID].task_timestamps[tasks_completed] = task_timestamp;
        characters[token_ID].user_addresses[tasks_completed] = msg.sender;

        characters[token_ID].tasks_completed = tasks_completed + 1;
    ...
    function completeTaskBatch(uint256[] memory task_IDs, string[] memory task_tags) onlyOwner public returns (bytes32[] memory completed_task_hashes) {
        ...
        for (uint256 i = 0; i < task_IDs.length; i++) {
            ...
            // Store the task hash in the result array
